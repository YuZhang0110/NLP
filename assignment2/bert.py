...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # Softmax over scores for normalization
    ...
    context_layer = context_layer.transpose(1, 2).contiguous().view(*context_layer.size()[:-2], self.all_head_size)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # Add-norm layer for attention output
    ...
    layer_output = self.out_dense(interm_output)

    # Another add-norm layer for feed forward output
    hidden_states = self.add_norm(attn_output, layer_output, lambda x: x, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    # Get token type ids and token type embeddings
    tk_type_ids = torch.zeros_like(input_ids)
